...
% Índices relevantes
input_indices = [1, 2, 3, 4,11]; % Inputs dos 4 atuadores
...
altitude_sys = ss(A, B_scaled, C(output_index, :), D_scaled);
...
for phase_margin = phase_margin_values
    for crossover_frequency = crossover_frequency_values
        try
            ...
            [altitudeController, ~] = pidtune(altitude_sys, 'PID', opts);
            ...
            if step_info.Overshoot >= min_overshoot && step_info.Overshoot >5 && step_info.Overshoot <10  && step_info.SettlingTime < 1.2
